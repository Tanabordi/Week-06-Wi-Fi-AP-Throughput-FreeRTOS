...
static esp_err_t api_data_handler(httpd_req_t *req) {
  ...
  snprintf(json_buf, sizeof(json_buf),
           ...
           snapshot.temperature, snapshot.humidity, snapshot.light_lux,
...
static esp_err_t dashboard_handler(httpd_req_t *req) {
  ...
  const char *html =
      ...
      "UI',sans-serif;background:#0f172a;color:#e2e8f0;min-height:100vh;"
      ...
      "  "
      ".subtitle{text-align:center;font-size:.8rem;color:#64748b;margin-bottom:"
      "24px;}"
      "  .cards{display:grid;grid-template-columns:1fr 1fr;gap:14px;}"
      "  "
      ".card{background:#1e293b;border-radius:16px;padding:20px;text-align:"
      "center;border:1px solid #334155;}"
      "  .card .icon{font-size:2rem;margin-bottom:8px;}"
      "  .card "
      ".label{font-size:.75rem;color:#94a3b8;text-transform:uppercase;letter-"
      "spacing:.05em;}"
      "  .card .value{font-size:2rem;font-weight:700;color:#f1f5f9;margin:4px "
      "0;}"
      ...
      "  <p class='subtitle'><span class='dot'></span>Lab 6-4 · FreeRTOS Queue "
      "+ HTTP Server</p>"
      "  <div class='cards'>"
      "    <div class='card'><div class='icon'>🌡️</div><div "
      "class='label'>Temperature</div>"
      "      <div class='value' id='temp'>--</div><div "
      "class='unit'>°C</div></div>"
      "    <div class='card'><div class='icon'>💧</div><div "
      "class='label'>Humidity</div>"
      "      <div class='value' id='hum'>--</div><div "
      "class='unit'>%</div></div>"
      "    <div class='card'><div class='icon'>☀️</div><div "
      "class='label'>Light</div>"
      "      <div class='value' id='lux'>--</div><div "
      "class='unit'>lux</div></div>"
      "    <div class='card'><div class='icon'>⏱️</div><div "
      "class='label'>Uptime</div>"
      "      <div class='value' id='ts'>--</div><div "
      "class='unit'>ms</div></div>"
      "  </div>"
      ...
      "    document.getElementById('updated').textContent='Last updated: '+new "
  ...
  ESP_LOGI(TAG_HTTP, "[FORENSIC]: HTML Dashboard sent (%d bytes)",
           strlen(html));
  return ESP_OK;
}
...
static void vSensorTask(void *pvParameters) {
  sensor_data_t data;
  ESP_LOGI(TAG_SENS, "[FORENSIC]: Sensor Collector Task started on Core %d",
           xPortGetCoreID());
  ...
  while (1) {
    ...
    ESP_LOGI(TAG_SENS,
             "[SENSOR TASK]: Pushing -> Temp: %.1f C, Hum: %.1f %%, Lux: %lu",
...
static void vNetworkTask(void *pvParameters) {
  ...
  ESP_LOGI(TAG_NET, "[FORENSIC]: Network Task started on Core %d",
           xPortGetCoreID());
...
static void wifi_init_softap(void) {
  ...
  ESP_LOGI(TAG_AP, "[FORENSIC]: SoftAP netif created at %p (IP: 192.168.4.1)",
           ap_netif);
  ...
  // --- WORKAROUND FOR BROWNOUT ---
  ESP_LOGI(TAG_AP, "[FORENSIC]: Disabling BOD and lowering TX power");
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG,
                 0); // Disable Brownout Detector temporarily

  ESP_LOGI(TAG_AP, "[FORENSIC]: Call esp_wifi_start()");
  ESP_ERROR_CHECK(esp_wifi_start());

  // Decrease TX power to prevent huge current spike (default is 78 = 19.5 dBm,
  // 20 = 5 dBm)
  ...
  ESP_LOGI(TAG_AP, "=======================================================");
  ESP_LOGI(TAG_AP, "  SoftAP Running! SSID: \"%s\", Channel: %d", AP_SSID,
