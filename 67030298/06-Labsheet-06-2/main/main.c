...
#define EXAMPLE_ESP_WIFI_SSID "ESP32_AP_PlengInwza007"
#define EXAMPLE_ESP_WIFI_PASS "3832538325"
...
void app_main(void) {
  // [SOFTWARE WORKAROUND] ปิด Brownout Detector ชั่วคราวเพื่อแก้ปัญหาไฟตกตอนเปิด Wi-Fi
  ...
  ESP_LOGI(
      TAG,
      "[FORENSIC]: SoftAP Interface created at %p (Default IP: 192.168.4.1)",
      ap_netif);
  ...
  ESP_LOGI(TAG,
           "[FORENSIC]: Call esp_wifi_set_config(WIFI_IF_AP, &wifi_config)");
  ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_AP, &wifi_config));

  ESP_LOGI(TAG, "[FORENSIC]: Call esp_wifi_start()");
  ESP_ERROR_CHECK(esp_wifi_start());

  // [LAB 6-2] ปรับลดกำลังส่งสัญญาณ Tx Power
  // 80 = 20 dBm (Max), 60 = 15 dBm, 40 = 10 dBm, 20 = 5 dBm, 8 = 2 dBm (Min)
